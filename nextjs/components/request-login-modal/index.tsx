...
export default function RedirectLoginModal() {
    ...
    >
      <DialogContent className='p-4 w-[350px] rounded-xl flex flex-col gap-4 items-center justify-center'>
        ...
        <div className='w-full flex justify-between gap-5'>
          <Link className='flex-1' href={callBackUrl ? `/register?callbackUrl=${callBackUrl}` : '/register'}>
            <Button
              variant='outline'
              size='lg'
          ...
          <Link className='flex-1' href={callBackUrl ? `/login?callbackUrl=${callBackUrl}` : '/login'}>
            <Button
              size='lg'
