...
import { ChangePasswordDto, ResetPasswordDto, VerifyFogotPasswordDto } from './dto/password.dto'
...
export class AuthenticationService {
  ...
  async register(registerDto: RegisterDto) {
    ...
    const verifyEmailToken = await this.generateJwtToken({ id: user.id }, TOKEN_TYPE.VERIFY_EMAIL)
  ...
  async sendMailForgotPassword(email: string) {
    ...
    const token = await this.generateJwtToken({ id: user.id }, TOKEN_TYPE.FORGOT_PASSWORD)
    // todo send email
    return { token }
  }
  ...
  async reSendMailForgotPassword(email: string) {
    ...
    const newToken = await this.generateJwtToken({ id: user.id }, TOKEN_TYPE.FORGOT_PASSWORD)
    // todo send email
