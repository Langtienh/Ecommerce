...
export default function Auth({ className }: AuthProps) {
  ...
  if (!user)
    return (
      <Link href='/login'>
        <Button variant='link' className={cn('bg-white bg-opacity-15 rounded-xl text-white', className)}>
        ...
        <DialogContent className='w-[420px] rounded-3xl left-0 sm:left-auto sm:right-3 top-16 translate-x-0 translate-y-0 bg-gray-300 p-4 pb-0'>
          ...
          <div className='flex flex-col items-center justify-center'>
            ...
            <div className='flex gap-1 w-full py-2 *:flex-1 *:w-full'>
              <Button asChild className='h-16 bg-gray-50 hover:bg-gray-300 text-back text-base rounded-s-3xl gap-3'>
                <Link href='/smember'>
            ...
            <p className='h-[60px] text-center text-xs flex items-center'>Privacy Policy - Terms of Service</p>
          </div>
        </DialogContent>
        <HoverCard>
          <HoverCardTrigger>
